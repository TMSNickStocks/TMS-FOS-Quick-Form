...
| `short-link.test.js` | 43 | short-link code generation and 80-bit entropy; Redis holds the sealed token only, with no plaintext client data; TTL bounded by 72h and by the token's own expiry; resolution, reuse, and every failure mode answering identically; rate limiting; **Redis-down, unconfigured and misconfigured fallback to the legacy link**; the real UPSTASH_REDIS_KV_REST_API_* names and their precedence over every alias; the read-only token never selected; legacy links unchanged; both modes; logging; CSRF and origin; routing and mobile layout; **email generation and Gmail delivery untouched** |
| **Total** | **287** | |
