...
| `short-link.test.js` | 38 | short-link code generation and 80-bit entropy; Redis holds the sealed token only, with no plaintext client data; TTL bounded by 72h and by the token's own expiry; resolution, reuse, and every failure mode answering identically; rate limiting; **Redis-down and unconfigured fallback to the legacy link**; legacy links unchanged; both modes; logging; CSRF and origin; routing and mobile layout; **email generation and Gmail delivery untouched** |
| **Total** | **282** | |
