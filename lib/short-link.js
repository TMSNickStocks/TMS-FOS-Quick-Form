...
// 30-day expiry, the reference gate or the questionnaire changes: once the
...
const { ttlHours, prefillExpiry, MAX_TTL_HOURS } = require('./prefill');

const KEY_PREFIX = 'fos-short:v1:';

// Redis may hold a code for at most 30 days, and never longer than the sealed
...
//
// The ceiling is the prefill module's own, not a second copy of the number: if
// the link lifetime is ever changed again, a mapping that outlived its token
// would be the one failure this cap exists to prevent.
const MAX_TTL_SECONDS = MAX_TTL_HOURS * 3600;
...
//
// The read-only token is deliberately absent from both lists: creating a link
// is a write, so UPSTASH_REDIS_KV_REST_API_READ_ONLY_TOKEN must never be
// selected. A test asserts no candidate name matches /READ_ONLY/.
const URL_VARS = [
  'UPSTASH_REDIS_KV_REST_API_URL',  // what the integration actually created here
...
// Never throws. The Upstash constructor rejects a malformed URL - a redis://
// value in a REST slot, say - and requiring the SDK can fail too. Either would
// otherwise escape createShortCode and fail the staff request with a 500, when
// the whole point is that staff still get a working legacy link. The outcome
// is cached so one bad configuration is diagnosed once, not on every call.
function store() {
  if (injectedStore) return injectedStore;
  if (cachedStore === null) {
    try {
      cachedStore = upstashStore() || false;
    } catch {
...
// never more than 30 days whatever the token says.
...
async function createShortCode(sealedToken) {
  ...
  try {
    // Obtaining the client sits inside the guard as well as the write, so a
    // configuration or construction failure degrades to the legacy link
