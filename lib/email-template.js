...
function resolveBlock(q, data) {
  ...
  if (q.kind === 'date') {
    // A question may offer an explicit "I don't know" alternative. When the
    // client chose it the record carries that answer and no date: nothing is
    // generated or inferred from a blank date field.
    if (q.unknownField && data[q.unknownField] === true) { base.answer = [q.unknownLabel]; return base; }
    base.answer = [dateLine(data[q.field])];
    return base;
  }
...
function blocksFor(data) {
  return {
    timebar: timebarBlocks(data).map((q) => resolveBlock(q, data)),
    fos: fosBlocks(data).map((q) => resolveBlock(q, data))
  };
}
...
module.exports = {
  ...
  poundsDisplay, moneyLine, dateLine,
