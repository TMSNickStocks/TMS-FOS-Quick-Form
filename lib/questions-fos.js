...
// Client-facing presentation heading for section 1 (approved 2026-09-15).
const VULNERABILITY_HEADING = '1. Your circumstances';
...
const FOS_QUESTIONS = [
  {
    ...
    // Recorded only when no substantive circumstance was selected - in
    // practice, when the client answered "None of these apply".
    //
    // When circumstances were selected, each gets its own block below naming
    // its own category, so repeating the aggregate list here would state the
    // same answer twice. The trade-off is deliberate: which categories applied
    // stays explicit (one block each), but which did NOT apply is now read from
    // the absence of a block rather than from a "not selected" line.
    applies: (d) => !hasVulnerabilityCategory(d)
  },
  ...
  ...VULNERABILITY_DETAILS.map((d) => ({
    id: d.id,
    // The section heading rides on every circumstance block, because any of
    // them may be the first one recorded. The record prints it once per run of
    // blocks that share it, so the section is always headed exactly once.
    heading: VULNERABILITY_HEADING,
    context: [d.category],
  ...
  {
    // Approved 2026-09-24. Asked only after Yes: a balance that was never
    // repaid has no repayment date, and an empty date field sitting under a No
    // is an invitation to put something in it.
    id: 'FOS_BALANCES_PAID_DATE',
    question: 'What date was the account repaid?',
    field: 'fosBalancesPaidDate',
    kind: 'date',
    applies: (d) => d.fosBalancesPaid === 'Yes'
  },
  {
    id: 'FOS_INCOME',
    // Client-facing presentation heading (approved 2026-09-15). The source's
...
module.exports = {
  ...
  VULNERABILITY_DETAILS, VULNERABILITY_DETAIL_QUESTION, VULNERABILITY_HEADING,
